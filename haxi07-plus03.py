...
def process_SNP():
    ...
    for snp_code, snp_info, left_reads1, right_reads2 in snp_data:
        ...
        snp_NUM[snp_info] = ""
        if index_1 != -1 and index_1 - match_num >= 0:
            substring_1 = left_reads1[index_1 - match_num:index_1]
            snp_binary_1 = convert_to_binary_P(substring_1)
            if snp_binary_1 in hash_table:
                res = MyStruct(snp_code, snp_info, "left_reads1_前侧", substring_1, 0)
        ...
        if index_3 != -1 and len(left_reads1) - index_3 - match_num - 1 >= 0:
            ...
            if snp_binary_1 in hash_table:
                ...
                res = MyStruct(snp_code, snp_info, "left_reads1_后侧", substring_1, 0)
                hash_table[snp_binary_1].append(res)
        if index_2 != -1 and index_2 - match_num >= 0:
            substring_2 = right_reads2[index_2 - match_num:index_2]
            snp_binary_2 = convert_to_binary_P(substring_2)
            if snp_binary_2 in hash_table:
                res = MyStruct(snp_code, snp_info, "right_reads2_前侧", substring_2, 0)
...
def match(file_path, filename):
    ...
    if os.path.isfile(file_path):
        ...
        for seq_id, seq_data in seq.items():
            ...
            length = len(sequence)
            if length - match_num - 2 >= 0:
                for i in range(1, length - match_num - 3, 1):
                    patch = sequence[i:i + match_num]
                    ...
                    if binary in hash_table:
                        for j in range(0, len(hash_table[binary]), 1):
                            if len(snp_NUM[hash_table[binary][j].gene_name]) < snp_limit:
                                if hash_table[binary][j].direction == "left_reads1_前侧":
                                    line = [hash_table[binary][j].snp_code, hash_table[binary][j].gene_name,
                                            ...
                                            i + match_num, sequence[i + match_num], Chain(sequence)]
                                    snp_NUM[hash_table[binary][j].gene_name] += sequence[i + match_num]
                                    ws.append(line)
                                ...
                                if hash_table[binary][j].direction == "right_reads2_前侧":
                                    line = [hash_table[binary][j].snp_code, hash_table[binary][j].gene_name,
                                            hash_table[binary][j].direction,
                                            hash_table[binary][j].gene_seq, seq_id,

                                            i + match_num, rep(sequence[i + match_num]), Chain(sequence)]
                                    snp_NUM[hash_table[binary][j].gene_name] += rep(sequence[i + match_num])
                                    ws.append(line)
